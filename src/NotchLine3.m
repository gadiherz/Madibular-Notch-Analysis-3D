...
%   - See the Supplementary Online Methods (SOM) for mathematical details.
    ...
    while LoopFlg
        ...
        while numel(FitSmp)<15 && d <=0.50 + 6*RelProg
            d=d+0.05;
            FitSmp = RdgPtsInd(RdgPtsDist<=d);
            ...
            RdgPtsInd = setdiff(RdgPtsInd,FitSmp(RdgPtsPrjDir<0.2));
    ...
    %     %which is best in line with previous direction, rotate if needed
        ...
        [~,ind] = sort(RdgPtsDist);
        ind = ind(1:100);
        GenRdgDir = RKDmin(RdgPtsInd(ind),:);
        GenDist = pdist2(Rotv(RotvRam(NchLn(end)),:),Rotv(RotvRam(RdgPtsInd(ind)),:))';
        Flp = acos(dot(repmat(PcDirVecPrv,size(GenRdgDir,1),1),GenRdgDir,2)) >0.5*pi;
        GenRdgDir(Flp,:) = -GenRdgDir(Flp,:);
        GenRdgDir = mean(GenRdgDir./GenDist)./vecnorm(mean(GenRdgDir./GenDist),2,2); 
        PcDirVecAngs = acos(dot(PcDirVec',repmat(GenRdgDir,3,1),2));
    ...
    %     FitDirVec = mean(RdgPtsVec)./vecnorm(mean(RdgPtsVec),2,2);
        ...
        Flp = acos(dot(repmat(PcDirVecPrv,size(RdgDirVec,1),1),RdgDirVec,2)) >0.5*pi;
        ...
        %1. Points vector, 2. Ridge, 3. Previous vector, 4. target, 5.
        %original target
        Vecs = [PcDirVec;RdgDirVec;PcDirVecPrv;TrgtDir;OTrgtDir];
        FlpVecs = find(dot(repmat(Vecs(3,:),2,1),Vecs(1:2,:),2)<0);
        Vecs(FlpVecs,:) = -Vecs(FlpVecs,:);
        for i = 1:5
            VecAg(:,i) = dot(repmat(Vecs(i,:),5,1),Vecs,2);
        end
        Ign = all(VecAg<0.1,1)';
        Vecs(Ign,:) = repmat([0,0,0],numel(find(Ign)),1);
        PcDirVec = sum([0.25.*Vecs(1,:);0.25.*Vecs(2,:);0.3.*Vecs(3,:);0.15.*Vecs(4,:);0.05.*Vecs(5,:)]); %[0.2,0.25,0.2,0.15,0.2],
...
%         end
        ...
        if min(CorCondDist)<=0.5
            NchLn = [NchLn,TrgtPt];
            LoopFlg = false;
        elseif RelProg <= 0.25 && min(CorCondDist) > 1.1*CorCondDistOld 
            LoopFlg = false;
        else
            CorCondDistOld = min(CorCondDist);
        end
